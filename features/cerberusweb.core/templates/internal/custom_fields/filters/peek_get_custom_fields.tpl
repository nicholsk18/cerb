...
<table width="500" style="margin-left:10px;display:{if $expanded}block{else}none{/if};" id="{$divName}">
	...
	<tr>
		<td valign="top" width="1%" nowrap="nowrap">
			...
			<label><input type="checkbox" id="chkGetField{$field_id}" name="rules[]" value="cf_{$field_id}" data-cerb-ui-toggle="fieldGetValue{$field_id}" {if !is_null($crit_field)}checked="checked"{/if}> {$field->name}:</label>
		</td>
		<td width="99%">
			...
			{if Model_CustomField::TYPE_SINGLE_LINE==$field->type || Model_CustomField::TYPE_MULTI_LINE==$field->type || Model_CustomField::TYPE_URL==$field->type}
				...
				<br>
				<input type="text" name="value_cf_{$field_id}" size="45" value="{$crit_field.value}" data-cerb-ui-change="chkGetField{$field_id}" style="width:95%;"><br>
			...
			{elseif Model_CustomField::TYPE_CURRENCY==$field->type}
				...
				<input type="text" name="value_cf_{$field_id}" size="12" value="{$crit_field.value}" data-cerb-ui-change="chkGetField{$field_id}">
			...
			{elseif Model_CustomField::TYPE_CHECKBOX==$field->type}
				<label><input type="radio" name="value_cf_{$field_id}" value="1" {if !is_null($crit_field) && 1==$crit_field.value}checked="checked"{/if} data-cerb-ui-change="chkGetField{$field_id}"> {'common.yes'|devblocks_translate}</label>
				<label><input type="radio" name="value_cf_{$field_id}" value="0" {if !is_null($crit_field) && 0==$crit_field.value}checked="checked"{/if} data-cerb-ui-change="chkGetField{$field_id}"> {'common.no'|devblocks_translate}</label>
			{elseif Model_CustomField::TYPE_DATE==$field->type}
				<i>between:</i><br>
				<input type="text" name="value_cf_{$field_id}_from" size="20" value="{$crit_field.from}" data-cerb-ui-change="chkGetField{$field_id}" style="width:95%;"><br>
				<i>and:</i><br>
				<input type="text" name="value_cf_{$field_id}_to" size="20" value="{$crit_field.to}" data-cerb-ui-change="chkGetField{$field_id}" style="width:95%;"><br>
